<div>
    ...
    @if(in_array($form['action'], ['view']))
        ...
        <div class="bg-white border shadow rounded-lg mt-8 p-4 sm:p-5">
            <div class="mb-4">
                ...
                <div class="block md:flex items-center mt-5 shadow-lg rounded-lg px-5 border-b rounded-t dark:border-gray-600 gap-5 pb-4">
                    <div>
                        <img src="{{$view[0]['template']['faculty']['image'] ? asset('storage/images/student/' . $view[0]['template']['faculty']['image']) : 'https://ui-avatars.com/api/?name='.$view[0]['template']['faculty']['firstname'] . ' ' . $view[0]['template']['faculty']['lastname'].'&length=2&bold=true&color=ff0000&background=random'}}" class="rounded-lg w-[150px] h-[150px]">
                    </div>
                    <div class="mb-3">
            ...
            <div class="mb-4 shadow-lg rounded-lg">
                <div class="p-4 text-sm text-blue-800 border-t border-r border-l rounded-t-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400">
                    <h1 class="text-1xl font-bold uppercase">Faculty's Subjects</h1>
                </div>
                <div class="border-b border-gray-200 dark:border-gray-700 px-2 mx-4">
                    <ul class="flex flex-wrap -mb-px text-sm font-medium text-center text-gray-500 dark:text-gray-400 mt-3">
                        @foreach ($view as $data)
                            <li class="me-2">
                                <a wire:click='changeTabs({{$data['template']['curriculum_template'][0]['subjects']['id']}})'
                                    class="inline-flex items-center justify-center p-2 border-b-2 rounded-t-lg group cursor-pointer
                                        @if ($tab == $data['template']['curriculum_template'][0]['subjects']['id'])
                                            border-sky-600 text-sky-600 hover:text-sky-600 hover:border-sky-600 dark:border-gray-300 dark:text-gray-300 dark:hover:text-gray-300
                                        @else
                                            border-transparent hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300 dark:hover:border-gray-300
                                        @endif">
                ...
                <div class="p-5">
                    @foreach ($view as $data_key => $redata)
                        @if ($tab == $redata['template']['curriculum_template'][0]['subjects']['id'])
                            <div class="p-4 mt-5 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-600" role="alert">
                            ...
                            <div class="relative my-8 w-100 sm:flex gap-3 justify-between">
                                <div class="flex">
                                    <a href="{{route('admin.programs.results', ['id' => $form['id'], 'action' => 'delete', 'faculty' => $form['faculty'], 'template' => $redata['template']['template_id'], 'subject' => $tab])}}" class="flex items-center gap-1 bg-red-500 text-white py-2 px-6 text-sm font-bold rounded-md w-full sm:w-100 justify-center mb-2">
                                        Delete Responses
                                    </a>
                                </div>
                                ...
                                @if ($display['comments'])
                                    <div class="col-span-12">
                                        ...
                                        <div class="relative overflow-hidden shadow rounded-lg mt-2 max-h-56">
                                            <table class="w-full text-sm text-left">
                                                <tbody>
                                                    @forelse ($redata['comments'] as $key => $comments)
                                                        <tr class="bg-white border-b">
                                    ...
                                    <div class="grid grid-cols-12 gap-4">
                                        <div class="col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-6 xl:col-span-6 2xl:col-span-4" wire:ignore>
                                            <div class="relative overflow-hidden shadow rounded-lg mt-2 h-100 p-4">
                                                ...
                                                <div id="chart-percentage-questions-{{$tab}}"></div>
                                            </div>
                                        </div>
                                        <div class="col-span-12 sm:col-span-12 md:col-span-12 lg:col-span-6 xl:col-span-6 2xl:col-span-4" wire:ignore>
                                            <div class="relative overflow-hidden shadow rounded-lg mt-2 h-100 p-4">
                                                ...
                                                <div id="chart-percentage-responses-{{$tab}}"></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <script type="module">
                                $(document).ready(function() {
                                    ...
                                    console.log(parse);
                                    setTimeout(() => {
                                        chart_fi(`#chart-percentage-questions-{{$tab}}`, parse.total_interpretation);
                                    }, 500);
                                    setTimeout(() => {
                                        chart_se(`#chart-percentage-responses-{{$tab}}`, parse.respondents);
                                    }, 500);
                                    Livewire.on('tabs', (data) => {
                                        const key = data.tab;
                                        setTimeout(() => {
                                            chart_fi(`#chart-percentage-questions-${key}`, data.data[0]['total_interpretation']);
                                        }, 500);
                                        setTimeout(() => {
                                            chart_se(`#chart-percentage-responses-${key}`, data.data[0]['respondents']);
                                        }, 500);
                                    });
                                });
                            </script>
    ...
    @elseif(in_array($form['action'], ['delete']))
        <div class="w-100 block md:flex justify-between items-center gap-2 mt-5">
            {{-- <div>
                <a wire:navigate href="{{route('admin.programs.results', ['id' => $form['id'], 'action' => 'view', 'faculty' => $form['faculty'], 'template' => $redata['template']['id'], 'subject' => $form['subject']])}}" class="bg-slate-900 py-2 px-6 text-white text-sm font-bold rounded-md">Go Back</a>
            </div> --}}
        </div>
        <div class="mt-10">
    ...
    @else
        ...
        <div class="mt-10 relative w-full h-full">
            <div class="relative sm:rounded-lg">
                @if(session()->has('no_questionnaire'))
                    <div class="flex items-center p-4 mb-4 text-sm text-red-800 border border-red-300 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400 dark:border-red-800" role="alert">
                        ...
                        <span class="sr-only">Info</span>
                        <div>
                        <span class="font-medium">{{session('no_questionnaire')}}</span>
                        </div>
                    </div>
                @endif
                                    ...
                                    <div wire:ignore.self id="drodown" class="dropdown absolute top-[100px] right-[100px] z-50 hidden bg-white divide-y divide-gray-100 rounded-lg shadow dark:bg-gray-700">
                                        <ul class="py-2 text-sm text-gray-700 dark:text-gray-200 w-full" aria-labelledby="dropdownDefaultButton">
                                            <li class="text-xs whitespace-nowrap">
                                                <a wire:navigate href="{{route('admin.programs.results', ['id' => $form['id'], 'action' => 'view', 'faculty' => $collection->id])}}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                    View Results
                                                </a>
                                            </li>
                                        </ul>
