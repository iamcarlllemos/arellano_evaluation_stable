    ...
    @if(in_array($form['action'], ['view']))
        ...
        <div class="bg-white border shadow rounded-lg mt-8 p-4 sm:p-8">
            ...
            <div class="relative my-8 w-100 sm:flex gap-3 justify-between">
                <div class="flex">
                    <a href="{{route('admin.programs.results', ['id' => $form['id'], 'action' => 'delete', 'faculty' => $form['faculty'], 'template' => $form['template'], 'subject' => $form['subject']])}}" class="flex items-center gap-1 bg-red-500 text-white py-2 px-6 text-sm font-bold rounded-md w-full sm:w-100 justify-center mb-2">
                        Delete Responses
                    </a>
                </div>
